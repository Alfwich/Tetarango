...
namespace AWTest
{
	...
	{
	public:
		static void run(std::shared_ptr<AW::SystemModuleBundle> modules)
		{
	...
	private:
		...
		static void luaTests(std::shared_ptr<AW::Lua> lua)
		{
			...
			public:
				...
				void onLuaCallback(const std::string& func, AW::LuaBoundObject* obj) override
				{
					...
					if (func == "getInArgs")
					{
						for (unsigned int i = 0; i < obj->args.size(); ++i)
						{
			...
			for (int i = 0; i < 1000; ++i)
			{
				...
				assert(!AWTest::test_ss.str().empty()); AWTest::test_ss.str("");
				assert(testObj->lastLuaCallbackObj->args.size() == 3);
				...
				assert(!AWTest::test_ss.str().empty()); AWTest::test_ss.str("");
				assert(testObj->lastLuaCallbackObj->args.size() == 4);
				...
				assert(!AWTest::test_ss.str().empty()); AWTest::test_ss.str("");
				assert(testObj->lastLuaCallbackObj->args.size() == 0);

				lua->registerGlobalFunction("testArgs", [](AW::LuaBoundObject* o) { for (unsigned int i = 0; i < o->args.size(); ++i) AWTest::test_ss << o->args[i]; });
		...
		static void memoryAndDeepTreeTests(std::shared_ptr<AW::SystemModuleBundle> modules)
		{

			while (true)
			{
				std::shared_ptr<AW::Scene> infScene = std::make_unique<AW::Scene>("second");
				{
					{
						std::shared_ptr<TestActor> to = std::make_unique<TestActor>();
						to->setScreenPosition(460.0, 140.0);
						infScene->add(to);
					}
					{
						std::shared_ptr<TestActor> to = std::make_unique<TestActor>();
				...
				std::shared_ptr<AW::ISerializable> o = modules->serialization->hydrate(t);

				std::shared_ptr<AW::Scene> infS = std::make_unique<AW::Scene>("inf");
				for (int i = 0; i < 1000; ++i)
				{
		...
		static void serializationTests(std::shared_ptr<AW::Serialization> serialization)
		{
		...
		static void textCompressionTests(std::shared_ptr<AW::Storage> storage)
		{
			{
		...
		static void encodingTests(std::shared_ptr<AW::Storage> storage)
		{
			{
		...
		{
			{
				const auto rng = std::make_shared<AW::RandomGenerator<int>>();
			...
			for (int k = 0; k < totalTestPasses; ++k)
			{
				...
				{
					const auto rng = std::make_shared<AW::RandomGenerator<int>>();
					const auto rng2 = std::make_shared<AW::RandomGenerator<int>>();
...

			{
				const auto rng = std::make_shared<AW::RandomGenerator<int>>();
				const auto rng2 = std::make_shared<AW::RandomGenerator<int>>();
				const auto rng3 = std::make_shared<AW::RandomGenerator<int>>();
